...
(function () {
  ...
  var FALLBACK = "Connection hiccup on my end — but the five-minute form right on this page reaches a human within 24 hours. Or email ceo@kitfire.ai.";
  ...
  var CSS = "#kfw{position:fixed;bottom:22px;right:22px;z-index:9999;font-family:'Inter',-apple-system,'Segoe UI',sans-serif}" +
    ...
    "#kfw input{flex:1;min-width:0;width:auto;background:#15151c;border:none;color:#f4f1ee;padding:12px;font-size:.9rem;outline:none;font-family:inherit}" +
  ...
  el.innerHTML = '<button class="bub" aria-label="Chat with Blaze, KitFire\'s AI assistant">🔥</button>' +
    ...
    '<p style="margin:0;padding:9px 14px;background:#241a12;color:#ffd9b8;font-size:12px;line-height:1.4">$299/month · AI automation access. Custom buildout quoted separately.</p><div class="log" role="log" aria-live="polite" aria-relevant="additions text"></div><div class="chips"></div>' +
  ...
  function send(v) {
    ...
    inp.value = "";
    if (COMMERCIAL.test(v)) { add('Approved pricing FAQ — ' + OFFER, 'ai'); hist.push({role:'assistant',content:OFFER}); return; }
    inp.disabled = true;
  ...
  var COMMERCIAL = /(?:[$€£]|\b(?:cost|price|pricing|fee|fees|payment|pay|paid|plan|plans|tier|tiers|budget|afford|buildout|setup|install|installed|installation|included|includes|include|subscription|monthly|per month)\b|what do i actually get)/i; ctrl.abort(); }, 20000) : null;
    fetch(API, {
      ...
      .then(function (d) {
        ...
        if (COMMERCIAL.test(d.reply)) d.reply = 'Approved pricing FAQ — ' + OFFER;
  ...
  el.querySelector(".hd button").onclick = function () { el.classList.remove("open"); el.querySelector(".bub").focus(); };
  el.addEventListener('keydown', function(e) { if(e.key === 'Escape') el.querySelector('.hd button').click(); });
  if (window.location.hash === '#blaze-chat') el.querySelector('.bub').click();
